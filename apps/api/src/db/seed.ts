...
async function seed() {
  ...
  try {
    // 1. System-provided Llama Local (default)
    ...
    await db.query(`
      ...
      VALUES ($1, $2, $3, $4, $5, $6)
    `, [llamaLocalId, 'llama-local', 'not-required', 'llama3.2', true, 'http://localhost:11434/v1']);

    // 2. Example: Ollama Cloud GLM-5 (user-configured)
    const glmId = uuidv4();
    await db.query(`
      INSERT INTO llm_settings (id, provider, api_key, model_name, is_default, base_url)
      VALUES ($1, $2, $3, $4, $5, $6)
    ...
    console.log('✅ LLM settings seeded (Llama Local + Ollama Cloud GLM-5)');
    ...
    await db.query(`
      ...
      VALUES ($1, $2, $3, $4)
    `, [agentId, 'Test Agent', 'You are a helpful assistant.', llamaLocalId]);
